...
type PriceData struct {
	...
	ResolvedPerRequestPricing *ResolvedPerRequestPricing
}
...
func (p *PriceData) ClearOtherRatios() {
	p.otherRatios = nil
}
...
func (p *PriceData) ToSetting() string {
	base := fmt.Sprintf("ModelPrice: %f, ModelRatio: %f, CompletionRatio: %f, CacheRatio: %f, GroupRatio: %f, UsePrice: %t, CacheCreationRatio: %f, CacheCreation5mRatio: %f, CacheCreation1hRatio: %f, QuotaToPreConsume: %d, ImageRatio: %f, AudioRatio: %f, AudioCompletionRatio: %f", p.ModelPrice, p.ModelRatio, p.CompletionRatio, p.CacheRatio, p.GroupRatioInfo.GroupRatio, p.UsePrice, p.CacheCreationRatio, p.CacheCreation5mRatio, p.CacheCreation1hRatio, p.QuotaToPreConsume, p.ImageRatio, p.AudioRatio, p.AudioCompletionRatio)
	if p.ResolvedPerRequestPricing == nil {
		return base
	}
	r := p.ResolvedPerRequestPricing
	return base + fmt.Sprintf(", ResolvedPerRequestPricing: {Mode: %s, MediaType: %s, Unit: %s, Resolution: %s, UnitPrice: %g, Quantity: %g, PriceUSD: %g, Quota: %d}", r.Mode, r.MediaType, r.Unit, r.Resolution, r.UnitPrice, r.Quantity, r.PriceUSD, r.Quota)
}
